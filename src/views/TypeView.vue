...
export default {
    ...
    methods: {
        ...
        getProjectsByType(slug, label) {
            if (label == "&laquo; Previous") {
                label = this.currentPage - 1
            } else if (label === "Next &raquo;") {
                label = this.currentPage + 1
            }
            this.url = `http://127.0.0.1:8000/api/types/${slug}/projects?page=${label}`;
            axios.get(this.url)
                .then(response => {
    ...
    watch: {
        '$route.params.slug': 'getProjectsByType'
    },
    mounted() {

        this.getProjectsByType(this.$route.params.slug)
        this.state.getTypes()
        ...
        <div class="container pb-5">
            ...
            <div class="row">
                                ...
                                <ul class="pagination">
                                    <li class="page-item" :class="link.active ? 'active' : ''" aria-current="page"
                                        ...
                                        <a class="page-link text-dark"
                                            ...
                                            @click="this.getProjectsByType(this.$route.params.slug, link.label)"
                                            v-html="link.label"></a>
                                    </li>
                                </ul>
                            </nav>
                ...
                <div class="col-12 col-md-3 order-first order-md-last text-center border-start">
                    ...
                    <div class="types d-none d-md-block pb-3">
                        <div class="card">
                            <div class="card-header bg-dark text-white">
                                <h5>Search by Category</h5>
                            </div>
                            ...
                            <div class="card-body">
                                <ul class=" list-unstyled">
                                    <li v-for="typ in this.state.types">
                                        <router-link :to="{ name: 'type', params: { slug: typ.slug } }"
                                            class="text-decoration-none" @click="getProjectsByType(typ.slug)"> {{
                                                typ.name }} </router-link>
                                    </li>
                                </ul>
                            </div>
